...
# Ensure user exists
user = session.query(models.User).filter(models.User.name == 'khushi').first()
if not user:
    user = models.User(name='khushi', email='khushi@example.com', role='User', hashed_password='')
    session.add(user)
    session.commit()
...
# Ensure book exists
book = session.query(models.Book).filter(models.Book.title == 'April 2026 Special').first()
if not book:
    book = models.Book(title='April 2026 Special', author='Admin', category='Contemporary Fiction', description='Allocated for Khushi on 30 March and due 6 April.')
    session.add(book)
    session.commit()
